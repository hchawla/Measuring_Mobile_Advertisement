...
def parse_pcap_file(filename, output_f):
	global ad_request_counter, ad_response_counter, ad_written_counter
...
#												raw_input('check mising click and destination url')
												...
												index2 = ad_json.find('"', index1+12)
												click_url = ad_json[index1+12:index2]
...
#						del response_dict[tcp_tuple]
	...
	f.close()
	print 'Current Ad Counter %d/%d, %d' % (ad_request_counter, ad_response_counter, ad_written_counter)
	if ad_request_counter > start_ad_request_counter:
		log_file = 'data_beta.log'
		log_f = open(log_file, 'a')
		string = '%s\t%d\t%d\t%d\n' % (filename, ad_request_counter-start_ad_request_counter, ad_response_counter-start_ad_response_counter, ad_written_counter-start_ad_written_counter)
		log_f.write(string)
		log_f.close()
